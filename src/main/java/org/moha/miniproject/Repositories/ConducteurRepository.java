...
public interface ConducteurRepository extends JpaRepository<Conducteur, Long> {
        ...
        @Query("SELECT c FROM Conducteur c " +
                        "WHERE c NOT IN (SELECT vo.conducteur FROM Voyage vo WHERE vo.conducteur = c) " +
                        "OR c NOT IN (SELECT vo.conducteur FROM Voyage vo WHERE :dateD <= vo.dateArrivee AND :dateA >= vo.dateDepart AND vo.conducteur != null)")
        List<Conducteur> getDisponibleConducteur(LocalDate dateD, LocalDate dateA);

        @Query("SELECT c FROM Conducteur c " +
                        "WHERE c NOT IN (SELECT vo.conducteur FROM Voyage vo WHERE vo.conducteur = c AND vo.idVoyage <> :idVoyage) " +
                        "OR c NOT IN (SELECT vo.conducteur FROM Voyage vo WHERE :dateD <= vo.dateArrivee AND :dateA >= vo.dateDepart AND vo.conducteur != null)")
